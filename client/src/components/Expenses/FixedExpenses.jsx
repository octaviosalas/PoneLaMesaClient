...
const FixedExpensesTable = ({expensesData, updateList}) => {
    ...
    const [loading, setLoading] = useState(true)
        ...
        const getExpensesDataAndCreateTable = async () => { 
            setLoading(true)
            console.log("salimo")
            try {
                ...
                cellRenderer: (cell) => {
                    ...
                    return (
                    <DeleteOrder type="expense" expenseData={item} updateExpenseList={getExpensesDataAndCreateTable}/>
                    );
                },
                });
        ...
        <div className='flex flex-col items-center justify-center mt-16 2xl:mt-12'>
         {loading ? <Loading/> : 
             columns.length !== 0 && data.length !== 0 && loading === false ? ( 
              ...
              ) : data.length === 0 && columns.length === 0 && filterIsOn === false && loading === false ? ( 
                <div className='flex flex-col items-center justify-center'>
                  <p className='font-medium text-black text-md '>No tenes gastos fijos en este momento</p>
                    <div className='flex items-center gap-6 mt-6'> 
                        <CreateExpense type="withOut" updateList={getExpensesDataAndCreateTable}/>
                    </div>
                </div>
              ) : null
          }
