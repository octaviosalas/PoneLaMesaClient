...
const MarkDepositArticlesAsFinished = ({depositData, updateNumbers}) => {
  ...
  const updateArticleStock = async () => { 
    if(newQuantity !== 0 && errorInQuantity !== true) { 
      const quantityToBeUpdated = ({ 
        newQuantity: depositData.quantity - newQuantity,
        quantity: Number(newQuantity),
        productId: depositData.productId
       });
  
       try {
          const updateStockAndUpdateDepositQuantity = await axios.put(`http://localhost:4000/deposit/updateQuantity/${depositData.productId}`, quantityToBeUpdated)
          console.log(updateStockAndUpdateDepositQuantity)
          if(updateStockAndUpdateDepositQuantity.status === 200) { 
    ...
    <>
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Reponer {depositData.productName}</ModalHeader>
              <ModalBody className="flex flex-col items-center justify-center">
                 <Input type="text" variant="underlined" label="Cantidad en Lavado" value={depositData.quantity - newQuantity}/>
...
export default MarkDepositArticlesAsFinished
